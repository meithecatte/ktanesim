...
pub struct Wires {
    state: ModuleState,
    rules: Arc<RuleSet>,
    wire_slots: [Option<Color>; MAX_WIRES],
...
impl Module for Wires {
    ...
    fn help_message(&self) -> &'static str {
        "`!!cut N` to cut the N-th wire. Example: `!!cut 6`."
    }
    ...
    fn view(&self, light: SolveLight) -> Render {
        let wire_slots = self.wire_slots;
        ...
        Render::with(move || {
            ...
            for ((wire, paths), cut) in wire_slots
                .iter()
                .zip(PATHS.iter())
                // skip slots without wires
...
type CairoPath = fn(&CairoContext);
/// Contains the paths for a wire slot (as functions taking a cairo context).
struct WirePaths {
    uncut: CairoPath,
    cut: CairoPath,
}
...
const PATHS: [WirePaths; MAX_WIRES] = [
    WirePaths {
