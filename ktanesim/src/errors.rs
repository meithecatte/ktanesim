...
impl ErrorMessage {
    pub fn title(&self) -> &'static str {
        match self {
            ...
            ModuleGroupStartsWithOp { .. } | ModuleGroupEndsWithOp { .. } | ModuleGroupTwoOpsInARow { .. } => "Syntax error",
            ...
            WireNumberZero | WireNumberTooLarge { .. } | WireNumberUnparseable(_) => "Invalid wire number",
        ...
        match self {
            ...
            BombAlreadyRunning { guild: true } => {
                "A bomb is already running in this channel. \
                 ...
                 or suggest detonating with `!detonate` (majority has to agree).".to_owned()
            }
            BombAlreadyRunning { guild: false } => {
                "A bomb is already running in this channel. \
                 If you don't want to defuse this bomb anymore, \
                 consider using `!detonate`.".to_owned()
            }
            NoLastView => {
            ...
            TrailingParameters(params) => {
                MessageBuilder::new()
                    .push("Unexpected parameters: ")
                    .push_mono_safe(params)
                    .build()
            }
            NoSuchGlobalCommand { command, input, last } => {
            ...
            NoSuchModuleCommand { command, module, hint } => {
                MessageBuilder::new()
                    .push_mono_safe(command)
            ...
            MissingModuleList => {
                "`!run` needs a module list specification.".to_owned()
            }
            MissingModuleCount { specifier } => {
                MessageBuilder::new()
            ...
            ModuleGroupTwoOpsInARow { input, index } => {
                MessageBuilder::new()
                    .push("The module group specifier ")
                    .push_mono_safe(input)
            ...
            RepeatedRuleSeedParameter => {
                "Please provide only one `ruleseed` parameter.".to_owned()
            }
            RepeatedTimerParameter => {
                "Please provide only one `timer` parameter.".to_owned()
            }
            RuleSeedParameterTooLarge => {
                format!(
            ...
            RuleSeedParameterUnparseable(input) => {
                MessageBuilder::new()
                    .push_mono_safe(input)
                    .push(" is not a valid rule seed. Try using `random` or a number.")
                    .build()
            }
            TimerParameterUnparseable(input) => {
                MessageBuilder::new()
                    .push_mono_safe(input)
                    .push(
